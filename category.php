...
// استعلام للحصول على اسم التصنيف
$select_category = $conn->prepare("SELECT category FROM categories WHERE id = :category");
$select_category->execute(['category' => $category]);
$category_name = $select_category->fetchColumn();

// استعلام المنتجات مع الترقيم
$select_products = $conn->prepare("SELECT * FROM products WHERE cat_id = :category LIMIT $limit OFFSET $offset");
...
<head>
  ...
  <style>
    ...
    .old-price {
      ...
      color: #999; /* لون شفاف للسعر القديم */
      font-size: 18px;
      margin-right: 10px;
    }
...
<body class="sub_page">
  ...
  <div class="container">
    ...
    <div class="pagination">
      ...
      $total_products = $conn->prepare("SELECT COUNT(*) FROM products WHERE cat_id = :category");
      $total_products->execute(['category' => $category]);
      ...
      for($i = 1; $i <= $total_pages; $i++) {
        ...
        echo "<a href='?id=$category&page=$i' class='page-link $active'>$i</a> ";
      }
      ?>
    </div>
  </div>
